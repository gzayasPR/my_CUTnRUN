...
# --- STEP 1: Filter BAMs (MAPQ ≥ 30, remove secondary, supplementary, unmapped) ---
# echo "[*] Filtering BAMs..."

# samtools view  -@ ${cpus} -b -q 30 -F 0x904 "$ECOLI_BAM" > "${SAMPLE_OUT}/${SAMPLE}_ecoli_filtered.bam"
# samtools view  -@ ${cpus} -b -q 30 -F 0x904 "$MOUSE_BAM" > "${SAMPLE_OUT}/${SAMPLE}_mouse_filtered.bam"
# samtools index "${SAMPLE_OUT}/${SAMPLE}_ecoli_filtered.bam"
# samtools index "${SAMPLE_OUT}/${SAMPLE}_mouse_filtered.bam"
...
# else
#     echo "[!] Skipping E. coli read count for ${SAMPLE} (no _H3 in sample name)"
# fi
