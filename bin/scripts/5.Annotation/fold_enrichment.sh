...
echo "🧮 Calculating log2FC BigWig..."
bigwigCompare \
  -b1 "$KO_BIGWIG" \
  -b2 "$WT_BIGWIG" \
  --operation log2 \
...
map_stats () {
  ...
  for metric in mean median sum stdev max min collapse ; do
    bedtools map -a "$peak_bed" -b "$WT_BEDG_SORTED" -c 4 -o $metric > "${OUTDIR}/${label}_WT_${metric}.bed"
    bedtools map -a "$peak_bed" -b "$KO_BEDG_SORTED" -c 4 -o $metric > "${OUTDIR}/${label}_KO_${metric}.bed"
  done
  bedtools map -a "$peak_bed" -b "$LOG2BEDG_SORTED" -c 4 -o mean > "${OUTDIR}/${label}_log2FC.bed"
  summary_file="${OUTDIR}/${label}_log2FC_full_summary.bed"
  rm "$summary_file"
  # Define metrics
  metrics=(mean median sum stdev max min)
  ...
  # Add log2FC column
  paste_cmd+=" <(awk '{print \$NF}' \"${OUTDIR}/${label}_log2FC.bed\")"
...
ml R
Rscript "${my_bin}/scripts/5.Annotation/combined_log2_summaries.R" $OUTDIR
Rscript "${my_bin}/scripts/5.Annotation/annotate_peaks.R" \
         $OUTDIR/combined_peaks_log2FC.bed \
         ${MOUSE_GTF} \
         $OUTDIR/combined_peaks_log2FC.csv \
         $OUTDIR
